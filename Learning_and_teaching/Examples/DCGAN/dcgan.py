...
class DCGAN(object):
    ...
    def __init__(self, path_to_save=None):
        ...
        with tf.control_dependencies(update_ops):
            self.generator_optimizer = tf.train.AdamOptimizer(learning_rate=0.0002, beta1=0.5).minimize(self.generator_loss + generator_regularization, var_list=generator_parameters)
            self.discriminator_optimizer = tf.train.AdamOptimizer(learning_rate=0.0002, beta1=0.5).minimize(self.discriminator_loss + discriminator_regularization, var_list=discriminator_parameters)
...
#            x = tf.contrib.layers.batch_norm(x, is_training=self.is_training, decay=momentum)
            x = tf.layers.dropout(x, self.keep_prob)
            x = tf.layers.conv2d(x, kernel_size=5, filters=512, strides=1, padding='same', activation=activation)
#            x = tf.contrib.layers.batch_norm(x, is_training=self.is_training, decay=momentum)
            x = tf.layers.dropout(x, self.keep_prob)
            x = tf.layers.conv2d(x, kernel_size=5, filters=1024, strides=1, padding='same', activation=activation)
#            x = tf.contrib.layers.batch_norm(x, is_training=self.is_training, decay=momentum)
#            x = tf.layers.dropout(x, self.keep_prob)
...
#        print('Discriminator loss: {}'.format(d_ls))
        ...
        if train_discriminator:
            self.sess.run([self.discriminator_optimizer], feed_dict={self.noise: noise, self.X_in: batch, self.keep_prob: keep_prob_train, self.is_training: True})
        if train_generator:
            self.sess.run([self.generator_optimizer], feed_dict={self.noise: noise, self.X_in: batch, self.keep_prob: keep_prob_train, self.is_training: True})
    
    def train(self, examples, epochs=1, batch_size=128):
        keep_prob_train = 0.5
        for e in range(epochs):
            for b in range(0,examples.shape[0],batch_size):
                ...
                if train_discriminator:
                    self.sess.run([self.discriminator_optimizer], feed_dict={self.noise: noise, self.X_in: batch, self.keep_prob: keep_prob_train, self.is_training:True})
                if train_generator:
                    self.sess.run([self.generator_optimizer], feed_dict={self.noise: noise, self.keep_prob: keep_prob_train, self.is_training: True})
